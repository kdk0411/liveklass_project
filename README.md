...
## 7. AWS 아키텍처 설계 및 서비스 역할

![AWS 아키텍처 구조](AWS architecture diagram.png)

### 1. 트래픽 처리 및 보안 계층
- Route 53  
  사용자가 입력한 도메인을 AWS 인프라로 연결하는 DNS 역할을 수행한다.

- AWS WAF  
  SQL Injection, 비정상 트래픽 등 악의적인 요청을 사전에 차단한다.

- Application Load Balancer (ALB)  
  사용자 요청을 여러 애플리케이션 컨테이너로 분산하며, L7 기반 라우팅을 수행한다.

---

### 2. 애플리케이션 실행 계층
- ECS Fargate (FastAPI App)  
  API 서버를 실행하는 메인 애플리케이션 컨테이너이며, 서버 관리 없이 운영된다.

- ECS Fargate (Telegraf)  
  시스템 및 애플리케이션 메트릭을 수집하여 시계열 데이터 저장소로 전달하는 역할을 한다.

- ECS Fargate RunTask (Generator)  
  EventBridge Scheduler를 통해 주기적으로 실행되는 배치 작업 컨테이너로, 데이터 생성 및 수집 작업을 수행한다.

Service와 RunTask의 차이는 다음과 같다:
- Service는 항상 실행되는 장기 프로세스(API 서버 등)에 적합하다.
- RunTask는 필요할 때만 실행되는 배치 작업에 적합하다.

---

### 3. 데이터 저장 계층
- Amazon RDS for PostgreSQL  
  사용자 정보 및 서비스 운영에 필요한 정형 데이터를 저장한다.

- Amazon Timestream for InfluxDB  
  시간 기반 메트릭 데이터를 저장하는 시계열 데이터베이스로, 모니터링 데이터에 최적화되어 있다.

두 데이터베이스의 역할은 다음과 같이 구분된다:
- RDS는 비즈니스 데이터를 저장한다.
- Timestream은 모니터링 및 메트릭 데이터를 저장한다.

---

### 4. 모니터링 계층
- Amazon CloudWatch  
  로그 수집 및 알람 설정을 통해 시스템 상태를 모니터링한다.

- Amazon Managed Grafana  
  수집된 메트릭 데이터를 시각화하여 대시보드 형태로 제공한다.

역할 차이는 다음과 같다:
- CloudWatch는 로그 및 메트릭 수집과 저장을 담당한다.
- Grafana는 데이터를 시각적으로 표현하는 역할을 한다.

---

### 5. 배포 및 보안 관리
- Amazon ECR  
  Docker 이미지를 저장하고 관리하는 컨테이너 레지스트리이다.

- AWS Secrets Manager  
  데이터베이스 비밀번호, API 키 등 민감한 정보를 안전하게 관리한다.
...
### 1. Telegraf 분리 여부
애플리케이션 내부에서 직접 메트릭을 전송하는 방식도 가능하지만, Telegraf를 별도의 서비스로 분리하였다.
...
- 결론  
  확장성과 유지보수성을 고려하여 별도 서비스로 분리하였다.

---

### 2. RunTask와 Service의 역할 분리
Generator 작업을 항상 실행할지, 필요할 때만 실행할지에 대한 고민이 있었다.

- 선택 방식  
  EventBridge Scheduler를 통해 RunTask로 실행
...
### 3. 시계열 데이터 저장소 선택
CloudWatch Metrics, Prometheus 등 다양한 선택지가 존재했으나 Timestream을 선택하였다.
...
본 아키텍처는 다음과 같은 목표를 기반으로 설계되었다.

- 서버리스 기반(Fargate)을 통한 운영 부담 최소화
- API, 배치, 모니터링의 역할 분리
- 트래픽 증가에 대한 확장성 확보
- 로그 및 메트릭 기반의 관측 가능성 확보
